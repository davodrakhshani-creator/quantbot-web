...
OUT=Path('data/dara_aug20_learned_replay_v3.json');START_EQUITY=100.;DAILY_STOP=-.01
...
def build_candidate(i,m,f5,f15,side,exz,mu,sd,base_threshold,ab,day_open,stage):
 ...
 # v3 quality: either materially stronger prototype or three independent primary pattern families.
 memory_strong=p>=.32
 confluence=primary>=3
 ...
 if not flow:stage['reject_flow']+=1;return None
 ...
 # Require real price response, not flow/pattern without movement.
 if max(r3,r6)<.00025:stage['reject_no_price_response']+=1;return None
 # Once the day has already extended 2%, ordinary trend-chasing is disabled.
 # Only triple-primary confluence can re-open the gate.
 if abs(dtd)>=.02 and not confluence:
  stage['reject_late_chase']+=1;return None
 x=v19.fused_candidate(i,m,f5,f15,side,p,wd,th)
 if not x:return None
 setup,s,stop,ei,score,diag,tp=x;diag=dict(diag)
 diag.update({'learned_v3':True,'memory_strong':memory_strong,'triple_primary_confluence':confluence,'day_state_live':ds,'dtd_pct':round(dtd*100,4),'r3_signed_pct':round(r3*100,4),'r6_signed_pct':round(r6*100,4),'instant_flow_aligned':True,'risk_pct_equity':pol['risk_pct_equity'],'risk_fraction_equity':pol['risk_fraction_equity'],'astro_day_bias':ab.bias,'astro_score':ab.score,'astro_aligned':None if ab.bias=='NEUTRAL' else side==ab.bias,'effective_prototype_threshold':th})
 score+=4*int(confluence)+2*int(memory_strong)
 stage['final']+=1
...
def main():
 ...
 keys=['seen','reject_base_memory','reject_quality','reject_flow','reject_vol_1_2','reject_counter_day','reject_no_price_response','reject_late_chase','final'];stage={k:0 for k in keys}
 opp=[]
 for i in inds:
 ...
 seq=[];eq=100.;i=inds[0];daily_stop=False;cooldown_until=-1
 while i<len(m)-2 and m[i]['t']<E:
  if eq<=99.:daily_stop=True;break
  ...
  # after a failed thesis/no-follow, enforce 5 closed minutes before re-entry to avoid fee churn.
  cooldown_until=sim[0]+6 if rr['net_pnl']<=0 else sim[0]+1
  i=sim[0]+1
 out={'version':'DARA-Aug20-LearnedReplay-v3','warning':'INTENTIONALLY IN-SAMPLE. v3 rules were derived from Aug20 v1/v2 results; not OOS evidence.','period_tehran':[START.isoformat(),END.isoformat()],'astro':asdict(ab),'learning_rules':['base prototype memory mandatory','quality gate: prototype p>=0.32 OR >=3 primary pattern families','flow sign aligned','volr 1-2 quarantined','live day-direction suppresses countertrend','require signed 3m/6m price response >=0.025%','after |DTD|>=2%, ordinary chase blocked; only triple-primary confluence allowed','5-minute cooldown after losing trade','risk/fee/TP logic unchanged'],'stage':stage,'opportunity':b0.stats(opp),'opportunity_by_side':b0.group_side(opp),'sequential':{'stats':b0.stats(seq),'by_side':b0.group_side(seq),'start_equity':100.,'final_equity':round(eq,6),'return_pct':round((eq/100-1)*100,4),'max_drawdown_pct':b0.max_drawdown(seq,100.),'daily_stop_hit':daily_stop,'trades':len(seq)},'trades':seq}
