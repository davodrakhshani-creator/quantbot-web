...
STATE=Path('data/v25_bybit_perp_l2_state.json')
...
def load_state():
    ...
    return {'version':'quantbot-v25-bybit-perp-l2','purpose':'FORWARD_ONLY_TRUE_PERPETUAL_L2_TAPE_ALPHA_AND_PAPER_NO_LIVE',
            ...
            'maker_candidates':0,'maker_fills':0,'paper_trades':[],'rejections':{},'created_at':utcnow()}
...
async def main():
    ...
    rejects=Counter(st.get('rejections',{}));last_event={(tier,s):-1e9 for tier in TIERS for s in(-1,1)};start=time.time();last_obs=0.0;ws_ok=False;sub_ok=False
    try:
        async with websockets.connect(WS,open_timeout=12,ping_interval=20,ping_timeout=20,max_size=2**23) as ws:
            ...
            while time.time()-start<RUN_SECONDS:
                ...
                sides={tier:side_for(hist,cur,t2,t5,cfg) for tier,cfg in TIERS.items()}
                if elapsed<=NEW_EVENT_CUTOFF_SEC and sub_ok:
                    for tier,side in sides.items():
                        if side and now-last_event[(tier,side)]>=COOLDOWN_SEC:
                            last_event[(tier,side)]=now;pending.append({'tier':tier,'side':side,'signal_ts':utcnow(),'wall_ts':now,'entry_mid':cur['mid'],'spread_bps':cur['spread_bps'],
                                'imb10':cur['imb10'],'microedge_bps':cur['microedge_bps'],'flow2':t2['flow'],'flow5':t5['flow'],'resolved':{},'mfe_bps':0.0,'mae_bps':0.0})
                # Pre-registered execution only on elite; no threshold tuning after outcomes.
                eside=sides['elite']
        ...
        st['ws_access_ok']=bool(ws_ok and sub_ok and st.get('orderbook_messages',0)>0 and st.get('trade_events',0)>0);st['subscription_ok']=sub_ok;st['rejections']=dict(rejects)
